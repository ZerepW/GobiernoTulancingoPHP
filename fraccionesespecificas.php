...
<head>
    ...
    <title>Fracciones Específicas</title>
...
<body>
    ...
    <div class="container">
        <h4>Obligaciones en Materia de Transparencia Específicas</h4><hr><br>
        <h5>I. En el caso del Poder Ejecutivo Estatal y los Municipios</h5>
        <table class="striped">
            ...
            <tbody>
                <?php // SELECCIONAR EQUIPO CPE
						$sql ="SELECT * FROM fraccion WHERE articulo = 70 ORDER BY articulo ASC";
        ...
        <h5>II. Adicionalmente, en el caso de los municipios</h5>
        <table class="striped">
            <thead class="purple lighten-1 white-text" >
                <tr>
                    <th colspan="2">Fracción</th>
            ...
            <tbody>
                <?php // SELECCIONAR EQUIPO CPE
						$sql ="SELECT * FROM fraccion WHERE articulo = 71 ORDER BY articulo ASC";
						$fracciones=mysql_query ($sql) or die(mysql_error());
						while ( $row=mysql_fetch_assoc($fracciones)) {
							$idFraccion = $row["idFraccion"];
							$articulo = $row["articulo"];
